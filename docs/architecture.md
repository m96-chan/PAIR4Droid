...
`telemetryValid` is true once two CPU samples exist. Whether PAIR actually consumes a manual
node's node-info for pressure is documented in `docs/pair-contract.md` §2; the design costs
nothing either way and is correct for full peers (Phase 2).

## Failure semantics

| Situation                    | OpenAI lane                         | Ollama lane                       | Why |
|-----------------------------|-------------------------------------|-----------------------------------|-----|
| unknown model               | 404 `{"error":{...,"code":"model_not_found"}}` | 404 `{"error":"model 'x' not found"}` | PAIR fails over to the next owner on 404 |
